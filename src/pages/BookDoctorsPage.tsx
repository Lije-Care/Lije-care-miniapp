...
export const BookDoctorsPage: FC = () => {
  ...
  const timeSlots = ["10:00 AM", "11:00 AM", "12:00 PM"];
    ...
    return (
      ...
      {/* Time Slot Selection */}
      <Text style={{ fontSize: "16px", fontWeight: "bold", margin: "15px 0" }}>
        Available Time Slot
      </Text>
      ...
      >
        {timeSlots.map((time) => (
          ...
          >{time}</SegmentedControlItem>
        ))}
      </SegmentedControl>
      <Button stretched style={{marginTop: '100px'}} onClick={()=> navigate('/payment')}>Book</Button>
    </div>
          </Page>
        );
